...
function therosessom_setup() {
	...
	register_nav_menus(
		array(
			'menu-1' => esc_html__( 'Primary', 'therosessom' ),
		)
	);
	...
	add_theme_support(
		'custom-background',
		apply_filters(
			'therosessom_custom_background_args',
			array(
...
function therosessom_scripts() {
	wp_enqueue_style( 'therosessom-tailwind', get_template_directory_uri() . '/build/main.css', array(), _S_VERSION );
	wp_style_add_data( 'therosessom-style', 'rtl', 'replace' );

	wp_enqueue_script( 'therosessom-navigation', get_template_directory_uri() . '/assets/js/navigation.js', array(), _S_VERSION, true );
